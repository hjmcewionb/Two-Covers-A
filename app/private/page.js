...
function PasswordGate({ onUnlocked }) {
  ...
  return (
    <div style={gateS.wrap}>
      <div style={gateS.box}>
        <div style={gateS.mark}>{"\u2014"}</div>
        <form onSubmit={submit}>
          <input type="password" value={pw} onChange={(e) => setPw(e.target.value)}
            placeholder="password" style={gateS.input} autoFocus />
          <button type="submit" disabled={busy || !pw} style={gateS.btn}>
            {busy ? "\u2026" : "Enter"}
          </button>
        </form>
        {err && <div style={gateS.err}>{err}</div>}
      </div>
    </div>
  );
}
...
const gateS = {
  wrap: { minHeight: "100vh", display: "flex", alignItems: "center",
    justifyContent: "center", background: "#FBF6EA", padding: 24 },
  box: { textAlign: "center", maxWidth: 320, width: "100%" },
  mark: { fontFamily: "'Fraunces', serif", fontSize: 40, color: "#D8C9A8",
    marginBottom: 18 },
  input: { width: "100%", boxSizing: "border-box", background: "transparent",
    border: "none", borderBottom: "1px solid #3A2C18", padding: "8px 4px",
    fontSize: 15, textAlign: "center", color: "#3A2C18",
    fontFamily: "'Jost', sans-serif", marginBottom: 16 },
  btn: { width: "100%", padding: "10px 16px", background: "#3A2C18",
    color: "#FBF6EA", border: "none", fontFamily: "'Fraunces', serif",
    fontStyle: "italic", fontSize: 15, cursor: "pointer", letterSpacing: 0.5 },
  err: { fontFamily: "'Fraunces', serif", fontStyle: "italic",
...
export default function Private() {
  ...
  useEffect(() => {
    if (typeof document !== "undefined") {
      const has = document.cookie.split(";").some((c) => c.trim().startsWith("tc_auth="));
      setAuthed(has);
    }
  }, []);
  ...
  return (
    <div style={S.app}>
      <header style={S.header}>
        ...
        <div style={S.mastRule} />
        <div style={S.initials}>AB &nbsp;&nbsp; RP</div>
      </header>
      ...
      <nav style={S.nav}>
        {NAV_ITEMS.map(({ k, label, Icon }) => {
          const active = tab === k;
          return (
            <button key={k} onClick={() => setTab(k)} aria-label={label}
...
function RecordsSection({ data }) {
  ...
  const toughest = catAvgs.length ? catAvgs.reduce((a, b) => (b.avg < a.avg ? b : a)) : null;
  const kindest = catAvgs.length ? catAvgs.reduce((a, b) => (b.avg > a.avg ? b : a)) : null;
  ...
  return (
    <section style={S.panel}>
      ...
      <div style={S.selectRow}>
        ...
        <Picker label="Compare with" value={selB} onChange={setSelB} data={data} allowNone />
      </div>
      <div style={{ marginTop: 18 }}>
...
function Picker({ label, value, onChange, data, allowNone }) {
  return (
    <label style={S.pickerWrap}>
      ...
      <select style={S.pickerSelect} value={value} onChange={(e) => onChange(e.target.value)}>
...
function ScatterSection({ data }) {
  ...
  const pts = data
    .map((e) => ({ name: e.name, x: e.scores?.[xCat], y: e.scores?.[yCat] }))
  ...
  return (
    <section style={S.panel}>
      ...
      <p style={S.subtleNote}>Each dot an establishment. See whether two measures move together.</p>
      <div style={S.selectRow}>
      ...
      {pts.length < 2 ? (
        ...
        <svg viewBox={`0 0 ${size} ${size}`}
          ...
          {[0, 2.5, 5, 7.5, 10].map((g) => (
            <g key={g}>
              <line x1={scale(g)} y1={pad} x2={scale(g)} y2={size - pad} stroke="#E4D8BD" strokeWidth="1" />
              <line x1={pad} y1={yScale(g)} x2={size - pad} y2={yScale(g)} stroke="#E4D8BD" strokeWidth="1" />
            </g>
          ))}
          <line x1={pad} y1={size - pad} x2={size - pad} y2={size - pad} stroke="#3A2C18" strokeWidth="1.5" />
          <line x1={pad} y1={pad} x2={pad} y2={size - pad} stroke="#3A2C18" strokeWidth="1.5" />
          {pts.map((p, i) => (
            <g key={i}>
              <circle cx={scale(p.x)} cy={yScale(p.y)} r="4.5" fill="#5C4326" />
              <text x={scale(p.x)} y={yScale(p.y) - 9} fontSize="8.5"
                fill="#6E5C42" textAnchor="middle" fontFamily="'Jost', sans-serif">{p.name}</text>
            </g>
          ))}
        </svg>
      )}
...
function TimeSection({ data }) {
  const dated = [...data].filter((e) => e.visit_date).sort((a, b) => a.visit_date.localeCompare(b.visit_date));
  const byYear = {};
  dated.forEach((e) => {
  ...
  return (
    <section style={S.panel}>
      ...
      {dated.length < 2 ? (
        ...
        <TimeLine points={dated.map((e) => ({ v: totalOf(e.scores), name: e.name }))} />
      )}
      {years.length > 0 && (
  ...
  return (
    <div>
      <svg viewBox={`0 0 ${w} ${h}`} style={{ width: "100%", height: "auto" }}>
        ...
        {points.map((p, i) => (
          <circle key={i} cx={x(i)} cy={y(p.v)} r="3" fill="#FBF6EA" stroke="#5C4326" strokeWidth="1.5" />
        ))}
      </svg>
